...
function generateNew() {
  ...
  if (twoCorrect.includes(hint3number)) {
    oneCorrectButWrongPlaced1[
      returnIndexNotEqual(
        ...
        twoCorrect.indexOf(hint3number)
      )
    ] = hint3number;
  } else {
    oneCorrectButWrongPlaced1[
      returnIndexNotEqual(correctCode.indexOf(hint3number))
    ] = hint3number;
  }
  ...
  if (twoCorrect.includes(hint4number)) {
    oneCorrectButWrongPlaced2[
      returnIndexNotEqual(
        ...
        twoCorrect.indexOf(hint4number)
      )
    ] = hint4number;
  } else {
    oneCorrectButWrongPlaced2[
      returnIndexNotEqual(correctCode.indexOf(hint4number))
    ] = hint4number;
  }

  //Generate Nothing Is Correct
  nothingIsCorrect[0] = returnNumberNotInArrays(correctCode, "", "");
  nothingIsCorrect[1] = returnNumberNotInArrays(
    correctCode,
    nothingIsCorrect,
    ""
  );
  nothingIsCorrect[2] = returnNumberNotInArrays(
    correctCode,
    nothingIsCorrect,
    ""
  );
  ...
  //Replace All Empty -1
  var allEmptyPlaces = [];
  twoCorrect.forEach(function (item, index, array) {
    if (item == -1) {
      allEmptyPlaces.push(["hint1", index]);
    }
  });
  oneCorrectAndWellPlaced.forEach(function (item, index, array) {
    if (item == -1) {
      allEmptyPlaces.push(["hint2", index]);
    }
  });
  oneCorrectButWrongPlaced1.forEach(function (item, index, array) {
    if (item == -1) {
      allEmptyPlaces.push(["hint3", index]);
    }
  });
  oneCorrectButWrongPlaced2.forEach(function (item, index, array) {
    if (item == -1) {
      allEmptyPlaces.push(["hint4", index]);
    }
  });

  for (i = 0; i < randomIntFrom(5, 5); i++) {
    var emptyToReplace =
      allEmptyPlaces[Math.floor(Math.random() * allEmptyPlaces.length)];
    ...
    if (arrayToReplaceFrom == "hint1") {
      ...
      oneCorrectAndWellPlaced[indexToReplace] = returnWrongNumberNotInArray(
        oneCorrectAndWellPlaced
      );
    } else if (arrayToReplaceFrom == "hint3") {
      oneCorrectButWrongPlaced1[indexToReplace] = returnWrongNumberNotInArray(
        oneCorrectButWrongPlaced1
      );
    } else if (arrayToReplaceFrom == "hint4") {
      oneCorrectButWrongPlaced2[indexToReplace] = returnWrongNumberNotInArray(
        oneCorrectButWrongPlaced2
      );
    }
  }
  ...
  for (i = 0; i < allEmptyPlaces.length + 2; i++) {
    ...
    if (arrayToReplaceFrom == "hint1") {
      twoCorrect[indexToReplace] = returnNumberNotInArrays(
        correctCode,
        nothingIsCorrect,
        twoCorrect
      );
    } else if (arrayToReplaceFrom == "hint2") {
      oneCorrectAndWellPlaced[indexToReplace] = returnNumberNotInArrays(
        correctCode,
        nothingIsCorrect,
        oneCorrectAndWellPlaced
      );
    } else if (arrayToReplaceFrom == "hint3") {
      oneCorrectButWrongPlaced1[indexToReplace] = returnNumberNotInArrays(
        correctCode,
        nothingIsCorrect,
        oneCorrectButWrongPlaced1
      );
    } else if (arrayToReplaceFrom == "hint4") {
      oneCorrectButWrongPlaced2[indexToReplace] = returnNumberNotInArrays(
        correctCode,
        nothingIsCorrect,
        oneCorrectButWrongPlaced2
      );
  ...
  hint2.innerHTML =
    oneCorrectAndWellPlaced[0] +
    " " +
    oneCorrectAndWellPlaced[1] +
    " " +
    oneCorrectAndWellPlaced[2];
  hint3.innerHTML =
    oneCorrectButWrongPlaced1[0] +
    " " +
    oneCorrectButWrongPlaced1[1] +
    " " +
    oneCorrectButWrongPlaced1[2];
  hint4.innerHTML =
    oneCorrectButWrongPlaced2[0] +
    " " +
    oneCorrectButWrongPlaced2[1] +
    " " +
    oneCorrectButWrongPlaced2[2];
  hint5.innerHTML =
    nothingIsCorrect[0] + " " + nothingIsCorrect[1] + " " + nothingIsCorrect[2];
}
